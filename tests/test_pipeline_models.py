...
class TestStackModel:
    ...
    def test_stack_model_with_config_field(self):
        ...
        assert stack_templated.config == Path("./configs/${{ inputs.environment }}/stack.yaml")
    ...
    def test_config_path_validation_rules(self):
        ...
        stack1 = StackModel.model_validate({**base_stack, "config": "configs/dev/app/samconfig.yaml"})
        assert stack1.config == Path("configs/dev/app/samconfig.yaml")
        
        # Valid: Explicit .yml file  
        stack2 = StackModel.model_validate({**base_stack, "config": "configs/dev/app/config.yml"})
        ...
        with pytest.raises(ValidationError, match="Invalid config path.*must end with"):
            StackModel.model_validate({**base_stack, "config": "configs/dev/app/config.json"})
        ...
        assert stack_templated.config == Path("configs/${{ inputs.environment }}/app/samconfig.yaml")
    ...
    def test_config_path_template_processing_and_validation(self):
        ...
        from samstacks.core import Pipeline, _validate_config_path
    ...
    def test_external_config_generation_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert config_content["default"]["deploy"]["parameters"]["stack_name"] == "staging-test-stack"
            assert config_content["default"]["deploy"]["parameters"]["region"] == "us-west-2"
            assert "Environment=staging" in config_content["default"]["deploy"]["parameters"]["parameter_overrides"]
            ...
            assert config_content["default"]["build"]["parameters"]["template"] == expected_template_path
            assert config_content["default"]["deploy"]["parameters"]["template"] == expected_template_path
